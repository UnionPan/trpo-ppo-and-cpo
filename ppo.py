...
class PPO:
    def __init__(self, args):
        ...
        self.network = ActorCritic(self.num_inputs, self.num_actions, layer_norm=self.args.layer_norm)
    ...
    def clipped_training(self, advantages, memory, returns_computed, i_episode):
        ...
        for i_epoch in range(int(self.args.num_epoch * batch_size / self.args.minibatch_size)):
            ...
            total_loss = loss_surr + self.args.loss_coeff_value * loss_value + self.args.loss_coeff_entropy * loss_entropy
    ...
    def collect_trajectories(self, i_episode):
        ...
        while num_steps < self.args.batch_size:
            ...
            for t in range(self.args.max_step_per_episode):
                ...
                action_mean, action_logstd, value = self.network(Tensor(state).unsqueeze(0))
                action, logproba = self.network.select_action(action_mean, action_logstd)
            ...
            self.reward_plot.append({'steps': self.global_steps, 'reward': reward_sum})
    ...
    def ppo_step(self, j_parallel):
        ...
        torch.save(self.network.state_dict(), './saved_model/ppo_ac_{}_{}'.format(self.args.env_name, j_parallel))
    ...
    def plot_figure(self, datestamp, record_dfs, reward_cols):
        ...
        record_dfs.to_csv(joindir(RESULT_DIR, 'ppo-record-{}-{}.csv'.format(self.args.env_name, datestamp)))
    ...
    # Plot
        ...
        plt.savefig(joindir(RESULT_DIR, 'ppo-{}-{}.pdf'.format(self.args.env_name, datestamp))) 
    ...
    def render_test(self, env):
        ...
        for t in range(10000):
            ...
            if self.args.state_norm:
                state = self.running_state(state)
            action_mean, action_logstd = self.network._forward_actor(Tensor(state).unsqueeze(0))
            action, logproba = self.network.select_action(action_mean, action_logstd)
            action = action.data.numpy()[0]
            next_state, reward, done, _ = self.env.step(action)
            state = next_state
